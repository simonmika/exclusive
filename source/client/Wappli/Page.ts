...
export class Page {
	...
	AddHeaderButton(text: string, url: string, left: boolean, linkType: string, icon?: string) {
		const button = document.createElement("a")
		const leftOrRight = left ? "ui-btn-left" : "ui-btn-right"
		button.classList.add("ui-btn", "ui-shadow", "ui-corner-all", leftOrRight)
		button.setAttribute("data-rel", linkType)
		button.href = url
		button.appendChild(document.createTextNode(text))
		if (this.initialized)
			this.header.appendChild(button)
		this.headerElements.push(button)
	}
	Show(): void {
