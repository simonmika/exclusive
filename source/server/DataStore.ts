...
export class DataStore {
	...
	static Initiate(configuration: Configuration) {
		DataStore.configuration = configuration
		DataStore.LoadContent()
		DataStore.LoadUsers(configuration.url)
	...
	private static LoadContent(): void {
		DataStore.Content = fs.readdirSync(DataStore.configuration.contentFolder).filter((folderName: string) => {
			return folderName[0] != "."
		})
	}
	private static LoadUsers(url: url.Url): void {
		const folders: string[] = fs.readdirSync(DataStore.configuration.usersFolder).filter((folderName: string) => {
			return folderName[0] != "."
		})
		folders.forEach(folder => {
			const userFolder = path.join(DataStore.configuration.usersFolder, folder)
			const meta = JSON.parse(fs.readFileSync(path.join(userFolder, "meta.json"), "utf-8"))
			const user = new User(this.configuration, (meta.company) || (meta.Company), (meta.contact) || (meta.Contact))
			const contents: string[] = fs.readFileSync(path.join(userFolder, "content.csv"), "utf-8").split("\n")
			user.Name = folder
			user.Path = DataStore.configuration.contentFolder
			user.Url = url.href
